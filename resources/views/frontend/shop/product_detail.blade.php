            ...
            <div class="col-lg-7">
                <div class="product-single__media" data-media-type="vertical-thumbnail">
                        ...
                        <div class="swiper-container">
                            <div class="swiper-wrapper">
                                ...
                                <div class="swiper-slide product-single__image-item">
                                    <img loading="lazy" class="h-auto" src="{{asset('product/image/' .$data->image)}}" width="674" height="674" alt="" />

                                    <a data-fancybox="gallery" href="{{asset('product/image/' .$data->image)}}" data-bs-toggle="tooltip" data-bs-placement="left" title="Zoom">
                                ...
                                <div class="swiper-slide product-single__image-item">
                                    <img loading="lazy" class="h-auto" src="{{ asset('product/gallery/' . $image) }}" width="674" height="674" alt="" />

                                    <a data-fancybox="gallery" href="{{ asset('product/gallery/' . $image) }}" data-bs-toggle="tooltip" data-bs-placement="left" title="Zoom">
                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <use href="#icon_zoom" />
                                        </svg>
                                    </a>
                    ...
                    <div class="product-single__thumbnail">
                        <div class="swiper-container">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto" src="{{ asset('product/image/' . $data->image) }}" width="104" height="104" alt="{{$data->name}}" /></div>
                                @foreach(json_decode($data->gallery, true) as $image)
                                <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto" src="{{ asset('product/gallery/' . $image) }}" width="104" height="104" alt="{{$data->name}}" /></div>
                                @endforeach
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-5">
                ...
                <div class="product-single__price">
                    <span class="current-price">
                        @if($data->sale_price)
                        <s>${{$data->price}}</s> ${{$data->sale_price}}
                        @else
                        <span class="money price">{{$data->price}}</span>
                        @endif
                    </span>
                </div>
                ...
                @if(Cart::instance('cart')->content()->where('id',$data->id)->count()> 0)
                <a href="{{route('cart.index')}}" class="btn btn-info mb-3">Go to cart</a>
                @else
                <form name="addtocart-form" method="post" action="{{route('cart.add')}}">
                    ...
                    <div class="product-single__addtocart">
                        ...
                        <input type="hidden" name="price" value="{{$data->sale_price == '' ? $data->price : $data->sale_price}}">
                ...
                @if(Cart::instance('wishlist')->content()->where('id',$data->id)->count()> 0)
                <a href="{{route('cart.index')}}" class="btn btn-info mb-3">Go to Wishlist</a>
                ...
                <div class="product-single__addtocart">
                    <form class="wishlist-form" method="post" action="{{ route('wishlist.add') }}">
                        ...
                        <input type="hidden" name="name" value="{{ $data->name }}">
                        <input type="hidden" name="price" value="{{ $data->sale_price == '' ? $data->price : $data->sale_price }}">
                        <button type="submit" class="menu-link menu-link_us-s add-to-wishlist d-none">Add to Wishlist</button>
                    </form>
                </div>

                <!-- Custom Wishlist Button that triggers form submit -->
                <div class="product-single__addtolinks">
                    <a href="javascript:void(0);" class="menu-link menu-link_us-s add-to-wishlist-trigger">
                ...
                <div class="swiper-wrapper">
                    ...
                    <div class="swiper-slide product-card">
                        <div class="pc__img-wrapper">
                            <a href="{{ route('shop.product_detail', $related_product->slug) }}">
                                <img loading="lazy" src="{{asset('product/image/' .$related_product->image)}}" width="330" height="400" alt="{{$related_product->name}}" class="pc__img">

                                @foreach(json_decode($related_product->gallery, true) as $image)
                                <img loading="lazy" src="{{ asset('product/gallery/' . $image) }}" width="330" height="400" alt="{{$related_product->name}}" class="pc__img pc__img-second">
                                @endforeach

                            </a>
                            @if(Cart::instance('cart')->content()->where('id',$related_product->id)->count()> 0)
                            <a href="{{route('cart.index')}}" class="pc__atc anim_appear-bottom  position-absolute border-0 text-uppercase fw-medium btn btn-info mb-3">Go to cart</a>
                            @else
                            <form name="addtocart-form" method="post" action="{{route('cart.add')}}">
                                @csrf
                                ...
                                <input type="hidden" name="price" value="{{$related_product->sale_price == '' ? $related_product->price : $related_product->sale_price}}">

                                <button type="submit" class="pc__atc btn anim_appear-bottom btn position-absolute border-0 text-uppercase fw-medium js-add-cart" data-aside="cartDrawer">Add to
                                    Cart</button>
                            </form>
                            @endif
                        </div>
                        ...
                        <div class="pc__info position-relative">
                            ...
                            <div class="product-card__price d-flex">
                                <span class="money price">
                                    ...
                                    <s>${{$related_product->price}}</s> ${{$related_product->sale_price}}
                                    @else
...
<script>
    $(document).on('click', '.add-to-wishlist-trigger', function(e) {
        ...
        // Find the wishlist form just above this button
        const form = $(this).closest('.product-single__addtolinks').prev('.product-single__addtocart').find('form.wishlist-form');
